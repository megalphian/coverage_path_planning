...
def find_transition_point(s_orig, theta, cut_origin):
    ...
    if y_s_min[1] >= cut_origin[1]:
        ...
        hyperplane = LineString(
            [(x_s_min[0], cut_origin[1]), (cut_origin[0], cut_origin[1])])
...
def find_cone_of_bisection(P, v):
    ...
    v_l_id = adjacency_dict[v][1]
    v_r_id = adjacency_dict[v][0]
    v_l = v_l_id[1]
    v_r = v_r_id[1]
    print("v: %s" % (v,))
...
#	p.show()
    ...
    shp_visib = shapely.geometry.polygon.orient(
        Polygon(zip(point_x, point_y)), -1)
...
def combine_chains(P, theta):
    ...
    for i in range(len(chains)-1):
        ...
        dest_chain = LineString(
            chains[cut_parameters[1]]+[chains[cut_parameters[1]][0]])
        ...
        if distance_to_v == 0:
            orig_chain_1 = []
            orig_chain_2 = orig_chain.coords[:]
        else:
...
def combine_two_adjacent_polys(p1, p2, e):
    ...
    if (p2_v_idx == 0) and (p2_w_idx == 1):
        ...
        right_chain = p2[p2_w_idx:]+p2[:p2_v_idx]
    elif p2_w_idx < p2_v_idx:
        right_chain = p2[p2_v_idx:]+p2[:p2_w_idx]

    print(left_chain)
    print(right_chain)
    lr_left = LinearRing(left_chain)
    lr_right = LinearRing(right_chain)
...
def find_cut_edge(p1, p2, v):
    ...
    print("SOMETHING WENT WRONG AND SHARED EDGE WAS NOT FOUND")
