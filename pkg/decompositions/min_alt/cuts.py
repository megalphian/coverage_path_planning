...
#		si = [s[i-1], s[i]]
    for si in s:
        ...
        try:
            ...
            # unable to make the cut, try the next
            continue
...
#	print("Closest Intersection object: %s"%(intersection,))
    ...
    point_x, point_y = vp_compute(v, P_vis)
...
#		return s_orig[y_s_max_idx]
        ...
        hyperplane = LineString(
            [(cut_origin[0], y_s_max[1]+1), (cut_origin[0], y_s_min[1]-1)])
...
def cut(line, distance):
    ...
    if distance <= 0.0 or distance >= line.length:
        ...
        print(line)
        print(distance)
        raise ValueError("ERROR: Cut beyond length")
