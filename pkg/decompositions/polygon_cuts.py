...
def decomposing_line_cut_by_splicing(P, v, w):
    ...
    if distance_to_w > distance_to_v:
        ...
        left_w_chain, right_w_chain = cut_linestring(
            right_v_cut, distance_to_w)
        ...
        left_v_chain, right_v_chain = cut_linestring(
            right_w_cut, distance_to_v)
...
def decomposing_poly_cut_by_set_op(P, v, w, epsilon=10e-2):
    ...
    v_r_displacements = [(distance_to_v+(i*epsilon)) %
                         chain.length for i in [1, 0, 2]]
    w_l_displacements = [distance_to_w+(i*epsilon) for i in [-1, -2, 0]]
    w_r_displacements = [(distance_to_w+(i*epsilon)) %
                         chain.length for i in [1, 0, 2]]
    ...
    def splice_polygon(dist_v, dist_w):
        ...
        if dist_w % chain.length > dist_v % chain.length:
            ...
            common = LineString(left_w_chain).difference(
                LineString(left_v_chain))
            ...
            common = LineString(left_v_chain).difference(
                LineString(left_w_chain))
    ...
    for i in range(len(v_l_displacements)):
        for j in range(len(w_l_displacements)):
            ...
            p_l, p_r = splice_polygon(
                v_l_displacements[i], w_r_displacements[j])
            ...
            p_l, p_r = splice_polygon(
                v_r_displacements[i], w_l_displacements[j])
            p_l_lr = LinearRing(p_l+[p_l[0]])
            p_r_lr = LinearRing(p_r+[p_r[0]])
